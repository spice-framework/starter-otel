...
func TestValidateReleaseToolAuthorization(t *testing.T) {
	...
	metadata := func(
		...
		includeDevelopmentTool bool,
		verifierRequirement string,
		includeVerifierTool bool,
	) string {
		requirements := make([]map[string]string, 0, 2)
		if developmentRequirement != "" {
			requirements = append(requirements, map[string]string{
				"Path": developmentModule, "Version": developmentRequirement,
			})
		}
		if verifierRequirement != "" {
			requirements = append(requirements, map[string]string{
				"Path": releaseVerifierModule, "Version": verifierRequirement,
			})
		}
		tools := make([]map[string]string, 0, 2)
		if includeDevelopmentTool {
			tools = append(tools, map[string]string{"Path": developmentTool})
		}
		if includeVerifierTool {
			tools = append(tools, map[string]string{"Path": releaseVerifierTool})
		}
		content, err := json.Marshal(map[string]any{"Require": requirements, "Tool": tools})
		if err != nil {
			t.Fatal(err)
		}
		return string(content)
	}
	valid := metadata(developmentVersion, true, releaseVerifierVersion, true)
	tests := []struct {
		name    string
		content string
		wantErr string
		...
		{name: "exact authorization", content: valid},
		{
			...
			content: metadata(developmentVersion, false, releaseVerifierVersion, true),
			wantErr: "exactly one " + developmentTool,
		},
		{
			name:    "wrong development version",
			content: metadata("v0.0.0-wrong", true, releaseVerifierVersion, true),
			wantErr: "require exactly " + developmentVersion,
		},
		{
			name:    "missing development requirement",
			content: metadata("", true, releaseVerifierVersion, true),
			wantErr: "must require " + developmentModule,
		},
		{
			name:    "missing verifier tool",
			content: metadata(developmentVersion, true, releaseVerifierVersion, false),
			wantErr: "exactly one " + releaseVerifierTool,
		},
		{
			name:    "wrong verifier version",
			content: metadata(developmentVersion, true, "v0.0.0-wrong", true),
			wantErr: "require exactly " + releaseVerifierVersion,
		},
		{
			name:    "missing verifier requirement",
			content: metadata(developmentVersion, true, "", true),
			wantErr: "must require " + releaseVerifierModule,
		},
...
func TestValidateReleaseArchiveParity(t *testing.T) {
	...
	writeParityTestArchive(t, central, "starter-otel_0.0.0-rehearsal/", "same", "")
	writeParityTestArchive(t, retained, "starter-otel_0.0.0-rehearsal/", "same", "")
	if err := validateReleaseArchiveParity(
		central,
		"starter-otel_0.0.0-rehearsal/",
		retained,
		"starter-otel_0.0.0-rehearsal/",
	); err != nil {
	...
	t.Run("rejects entry drift", func(t *testing.T) {
		...
		writeParityTestArchive(t, drifted, "starter-otel_0.0.0-rehearsal/", "different", "")
	...
	t.Run("rejects unexpected retained root", func(t *testing.T) {
		...
		writeParityTestArchive(t, wrong, "starter-otel-0.0.0-rehearsal/", "same", "")
		err := validateReleaseArchiveParity(
			central,
			"starter-otel_0.0.0-rehearsal/",
			wrong,
	...
	t.Run("rejects hidden decompressed tail", func(t *testing.T) {
		...
		prefix := "starter-otel_0.0.0-rehearsal/"
	...
	t.Run("rejects raw compressed suffix", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "raw-suffix.tar.gz")
		prefix := "starter-otel_0.0.0-rehearsal/"
		writeParityTestArchive(t, wrong, prefix, "same", "")
		appendParityRawTail(t, wrong, []byte("raw trailing bytes"))
		if _, err := readParityArchive(wrong, prefix); err == nil ||
			!strings.Contains(err.Error(), "compressed archive has") {
			t.Fatalf("readParityArchive(raw suffix) error = %v", err)
		}
	})

	t.Run("rejects oversized compressed input before reading", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "oversized.tar.gz")
		file, err := os.Create(wrong) // #nosec G304 -- test path is inside t.TempDir.
		if err != nil {
			t.Fatal(err)
		}
		...
		if _, err := readParityArchive(wrong, "starter-otel_0.0.0-rehearsal/"); err == nil ||
			!strings.Contains(err.Error(), "exceeds") {
			t.Fatalf("readParityArchive(oversized) error = %v", err)
		}
	})
}

func TestReadReleaseArtifactRejectsNonRegularAndOversized(t *testing.T) {
	t.Parallel()
	t.Run("directory", func(t *testing.T) {
		...
		if _, err := readReleaseArtifact(root, "metadata"); err == nil ||
			!strings.Contains(err.Error(), "not a regular file") {
			t.Fatalf("readReleaseArtifact(directory) error = %v", err)
		}
	})
	t.Run("oversized", func(t *testing.T) {
		...
		root := t.TempDir()
		name := "metadata"
		file, err := os.Create(filepath.Join(root, name)) // #nosec G304 -- test path is inside t.TempDir.
		if err != nil {
			t.Fatal(err)
		}
		if err := file.Truncate(maximumParityMetadataSize + 1); err != nil {
			t.Fatal(err)
		}
		if err := file.Close(); err != nil {
			t.Fatal(err)
		}
		if _, err := readReleaseArtifact(root, name); err == nil ||
			!strings.Contains(err.Error(), "exceeds") {
			t.Fatalf("readReleaseArtifact(oversized) error = %v", err)
		}
	})
}
...
func TestValidateReleaseSBOMParity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "wrong OpenTelemetry provenance",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
				retained.CreationInfo.Creators[0] = "Organization: Spice Framework"
			},
			wantErr: "OpenTelemetry builder",
		},
		{
...
func paritySBOMFixtures() (releaseSBOM, releaseSBOM) {
	...
	central := common
	central.Name = "starter-otel " + rehearsalVersion
	...
	retained := common
	retained.Name = "Spice OpenTelemetry starter " + rehearsalVersion
	retained.DocumentNamespace = parityNamespace("", 'b')
	retained.CreationInfo.Creators = []string{
		"Organization: Spice Authors",
		"Tool: github.com/spice-framework/starter-otel/cmd/starter-otel-release",
	}
...
func appendParityRawTail(t *testing.T, filename string, content []byte) {
	t.Helper()
	file, err := os.OpenFile(filename, os.O_APPEND|os.O_WRONLY, 0) // #nosec G304 -- test path is inside t.TempDir.
	if err != nil {
		t.Fatal(err)
...
func releaseArchiveName() string {
	return "starter-otel_" + strings.TrimPrefix(rehearsalVersion, "v") + "_source.tar.gz"
}

func releaseSBOMName() string {
	return "starter-otel_" + strings.TrimPrefix(rehearsalVersion, "v") + "_sbom.spdx.json"
}

func parityNamespace(extra string, digit rune) string {
	return "https://github.com/spice-framework/starter-otel/releases/" + rehearsalVersion +
