...
func releaseRehearsal(ctx context.Context, root string) error {
	...
	parent, err := os.MkdirTemp("", repository+"-release-rehearsal-*")
	if err != nil {
	...
	plan, err := capture(ctx, root, offlineVendor, "go", "tool", developmentTool,
		"library-release", "plan", "--root="+root, "--repo="+repository,
...
func releaseRepositoryName() string {
	return strings.TrimPrefix(modulePath, "github.com/spice-framework/")
}
...
func validateReleaseRehearsal(root string, artifacts map[string][sha256.Size]byte) error {
	base := releaseRepositoryName() + "_" + strings.TrimPrefix(rehearsalVersion, "v")
...
func validateReleaseSBOM(content []byte) error {
	...
	repository := releaseRepositoryName()
	prefix := "https://github.com/spice-framework/" + repository + "/releases/" + rehearsalVersion + "/spdx/v1/"
	if sbom.SPDXVersion != "SPDX-2.3" || sbom.DataLicense != "CC0-1.0" || sbom.SPDXID != "SPDXRef-DOCUMENT" ||
		sbom.Name != repository+" "+rehearsalVersion || !validSBOMNamespace(sbom.DocumentNamespace, prefix) ||
