...
func Build(ctx context.Context, config Config) (result Result, resultErr error) {
	...
	staging, err := os.MkdirTemp(filepath.Dir(normalized.OutputDir), ".starter-otel-release-*")
	if err != nil {
...
func buildArtifacts(ctx context.Context, config Config, staging string) ([]string, error) {
	...
	archiveName := "starter-otel_" + versionName + "_source.tar.gz"
	...
	sbomName := "starter-otel_" + versionName + "_sbom.spdx.json"
