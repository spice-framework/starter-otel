...
func TestBuildIsDeterministicAndVerifiable(t *testing.T) {
	...
	for _, output := range outputs {
		...
		want := []string{
			...
			"starter-otel_1.2.3-rc.1+build.7_sbom.spdx.json",
			"starter-otel_1.2.3-rc.1+build.7_source.tar.gz",
		}
	...
	verifySourceArchive(t, filepath.Join(outputs[0], "starter-otel_1.2.3-rc.1+build.7_source.tar.gz"), epoch)
	verifySBOM(t, filepath.Join(outputs[0], "starter-otel_1.2.3-rc.1+build.7_sbom.spdx.json"), commit, epoch)
}
...
func TestBuildRejectsStaleCommittedGraphs(t *testing.T) {
	...
	t.Run("stale vendor", func(t *testing.T) {
		...
		for _, entry := range entries {
			if strings.HasPrefix(entry.Name(), ".starter-otel-release-") {
...
func verifySourceArchive(t *testing.T, name string, epoch time.Time) {
	...
	for {
		...
		found = true
		if !strings.HasPrefix(header.Name, "starter-otel_1.2.3-rc.1+build.7/") || filepath.IsAbs(header.Name) {
...
func verifySBOM(t *testing.T, name, commit string, epoch time.Time) {
	...
	if strings.Contains(string(readFile(t, name)), `:\`) || !strings.Contains(document.DocumentNamespace, "starter-otel") {
