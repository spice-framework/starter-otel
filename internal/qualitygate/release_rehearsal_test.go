...
func TestDeterministicReleaseArtifacts(t *testing.T) {
	...
	first := t.TempDir()
	second := t.TempDir()
	writeRehearsalTestFile(t, first, "artifact", "same")
	writeRehearsalTestFile(t, second, "artifact", "same")
	if _, err := deterministicReleaseArtifacts([]string{first, second}); err != nil {
		t.Fatalf("deterministicReleaseArtifacts() error = %v", err)
	}
	writeRehearsalTestFile(t, second, "artifact", "different")
...
func TestValidateReleaseRehearsal(t *testing.T) {
	...
	t.Run("rejects checksum mismatch", func(t *testing.T) {
		...
		writeRehearsalTestFile(t, corrupt, "checksums.txt", strings.Repeat("0", sha256.Size*2)+"  "+releaseSBOMName()+"\n"+strings.Repeat("0", sha256.Size*2)+"  "+releaseArchiveName()+"\n")
	...
	t.Run("rejects provenance drift", func(t *testing.T) {
		...
		writeRehearsalTestBytes(t, corrupt, releaseSBOMName(), content)
...
func writeReleaseFixture(t *testing.T) (string, map[string][sha256.Size]byte) {
	t.Helper()
	root := t.TempDir()
	repository := releaseRepositoryName()
	sbom := releaseSBOM{
		SPDXVersion: "SPDX-2.3", DataLicense: "CC0-1.0", SPDXID: "SPDXRef-DOCUMENT",
		Name:              repository + " " + rehearsalVersion,
		DocumentNamespace: "https://github.com/spice-framework/" + repository + "/releases/" + rehearsalVersion + "/spdx/v1/" + strings.Repeat("a", sha256.Size*2),
		...
		Packages:          []map[string]any{{"SPDXID": "SPDXRef-Package", "name": repository}},
	...
	writeRehearsalTestBytes(t, root, releaseSBOMName(), content)
	writeRehearsalTestFile(t, root, releaseArchiveName(), "source")
...
func copyReleaseFixture(t *testing.T, source, destination string) {
	...
	for _, name := range []string{"checksums.txt", releaseSBOMName(), releaseArchiveName()} {
		...
		writeRehearsalTestBytes(t, destination, name, content)
	}
}

func releaseArchiveName() string {
	return releaseRepositoryName() + "_" + strings.TrimPrefix(rehearsalVersion, "v") + "_source.tar.gz"
}

func releaseSBOMName() string {
	return releaseRepositoryName() + "_" + strings.TrimPrefix(rehearsalVersion, "v") + "_sbom.spdx.json"
}

func writeRehearsalTestFile(t *testing.T, root, name, content string) {
	t.Helper()
	writeRehearsalTestBytes(t, root, name, []byte(content))
}

func writeRehearsalTestBytes(t *testing.T, root, name string, content []byte) {
	t.Helper()
