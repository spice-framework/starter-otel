...
func releaseParity(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-otel-release-parity-*")
	if err != nil {
...
func validateReleaseParity(
	...
	base := "starter-otel_" + strings.TrimPrefix(rehearsalVersion, "v")
	...
	archivePrefix := "starter-otel_" + strings.TrimPrefix(rehearsalVersion, "v") + "/"
...
func readParityArchive(filename string, expectedPrefix string) (parityArchive, error) {
	content, err := readBoundedRegularPath(filename, maximumParityArchiveSize)
	if err != nil {
...
func validateReleaseSBOMParity(centralContent, retainedContent []byte) error {
	...
	baseNamespace := "https://github.com/spice-framework/starter-otel/releases/" +
...
func validateCentralSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "starter-otel "+rehearsalVersion ||
...
func validateRetainedSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "Spice OpenTelemetry starter "+rehearsalVersion ||
		...
		return errors.New("retained release SBOM provenance does not match the OpenTelemetry builder")
	}
	return nil
}
...
func readReleaseArtifact(rootPath string, name string) (_ []byte, resultErr error) {
	...
	content, err := readBoundedRegular(file, name, maximumParityMetadataSize)
	if err != nil {
...
func readBoundedRegularPath(filename string, maximum int64) (_ []byte, resultErr error) {
	file, err := os.Open(filename) // #nosec G304 -- caller supplies a generated temporary artifact path.
	if err != nil {
		return nil, err
	}
	defer func() { resultErr = errors.Join(resultErr, file.Close()) }()
	return readBoundedRegular(file, filename, maximum)
}

func readBoundedRegular(file *os.File, name string, maximum int64) ([]byte, error) {
	info, err := file.Stat()
	if err != nil {
		return nil, err
	}
